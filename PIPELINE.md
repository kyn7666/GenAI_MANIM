...
2. **에러 핸들링 강화**
   - ✅ **구현 완료**: Manim 실행 오류 자동 재시도 (최대 3번)
   - ✅ **구현 완료**: 에러 메시지 기반 LLM 피드백
   - ✅ **구현 완료**: Template renderer Fallback 메커니즘
   - IR 검증 스키마 강화

3. **추가 Template Renderers**
   - `render_graph.py` (BFS, DFS)
   - `render_tree.py` (Binary Tree)
   - `render_dp.py` (Dynamic Programming)

### 10.2 중기 목표 (1-2개월)

1. **Template 기반 렌더링 확대**
   - 모든 주요 도메인에 Template 제공
   - LLM은 IR 생성만 담당
   - 98%+ 성공률 달성
...
1. **Multi-stage IR 아키텍처 확립**
   - Pseudocode IR → Animation IR → Manim Code
   - 각 단계 검증 및 디버깅 가능
   - 재사용 가능한 중간 표현

2. **Hybrid Rendering 시스템**
   - Template 기반 (안정성) + LLM 기반 (유연성)
   - 도메인별 최적 전략 선택

3. **자동 품질 보장**
   - 색상 검증 및 자동 수정
   - 구문 검증 및 자동 수정
   - Reference template 기반 few-shot learning
   - **자동 재시도 메커니즘**: 실행 오류 시 에러 피드백으로 3번 재시도
   - **Fallback 시스템**: 재시도 실패 시 Template renderer 자동 전환
...
**현재 수준**: 
- 렌더링 성공률: **94%** (재시도 메커니즘 적용)
- 평균 생성 시간: **25초** (재시도 포함 시 최대 40초)
- Template 도메인: **98-100%** 성공률

**목표 수준**: 98%+ 성공률, Template 기반 확대

향후 Template Renderer 확대 및 품질 자동화를 통해 **교육용 알고리즘 시각화의 표준 플랫폼**으로 발전시킬 계획입니다.

---

## 부록 A: Animation IR 전체 예시
...
```json
{
  ...
  "actions": [
    {"step": 1, "target": "input_matrix", "animation": "fade_in", "description": "Show input image"},
    {"step": 2, "target": "kernel", "animation": "fade_in", "description": "Show convolution kernel"},
    {"step": 3, "target": "convolution_op", "animation": "fade_in", "description": "Convolution operation"},
    {"step": 4, "target": "kernel", "animation": "move", "description": "Slide kernel over input"},
    {"step": 5, "target": "feature_map", "animation": "fade_in", "description": "Generate feature map"}
  ]
}
```
...
```json
{
  ...
  "actions": [
    {"step": 1, "target": "array", "animation": "fade_in", "description": "Initialize array"},
    {"step": 2, "target": "array[0]", "animation": "highlight", "description": "Compare 5 and 2"},
    {"step": 3, "target": "array[0]", "animation": "swap", "description": "Swap 5 and 2"},
    {"step": 4, "target": "array[1]", "animation": "highlight", "description": "Compare 5 and 8"},
    {"step": 5, "target": "array[2]", "animation": "highlight", "description": "Compare 8 and 1"},
    {"step": 6, "target": "array[2]", "animation": "swap", "description": "Swap 8 and 1"}
  ]
}
```
